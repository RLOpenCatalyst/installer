...
seeddataloader() {
        ...
        sudo sed -i "s/abc/$hostname1/g" /vagrant/seeddata/EvalSetup.json
		sudo cp /opt/catadmin.pem /vagrant/seeddata/Newfiles
		sudo cp /opt/phoenix-validator.pem /vagrant/seeddata/Newfiles
		cd /vagrant/seeddata
		npm install
		cd ~
        /usr/bin/node /opt/seeddata/seedData.js /opt/seeddata/EvalSetup.json > /tmp/seeddata.log
}
